...
- src/data.py: CIFAR10 and ImageFolder dataloaders.
...
python src/train.py --model autoencoder --epochs 5
python src/train.py --model vit --epochs 5
python src/evaluate.py --model vit

By default the code uses CIFAR10 through torchvision, so it works without manually collecting images. You can also point the loader to an ImageFolder dataset.
