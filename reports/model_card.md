...
Default pipeline downloads CIFAR10 via torchvision, splits the official training set into train and validation subsets, and evaluates on the official held-out test set. The same loader supports ImageFolder datasets for custom image classification.
...
The evaluation script exports classification report and confusion matrix CSV files from the held-out test loader. Autoencoder quality should be evaluated with reconstruction loss and visual inspection.
...
- Compare CNN and ViT accuracy over 3, 10, and 25 epochs.
